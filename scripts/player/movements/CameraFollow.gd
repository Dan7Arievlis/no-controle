...
func set_player(_player):
	player = _player
	camera_stats = _player.camera_stats
...
func _look_ahead(delta: float):
	...
	if player.wall_jumping_timer <= -0.60 or not player.wall_jump_stats.enable_wall_jump: #or player.is_on_floor():
		...
		if camera_stats.increase_with_speed:
			position.x = lerpf(position.x, clamp(camera_stats.look_ahead.x * player.input_direction * \
				(abs(player.velocity.x) / player.movement_stats.walk_speed), -size.x, size.x), camera_stats.damping.x * delta)
		else:
			position.x = lerpf(position.x, clamp(camera_stats.look_ahead.x * player.input_direction, \
		...
		if focus_player:
			...
			if player.velocity.y >= 0.0:
				position.y = lerpf(position.y, clamp(camera_stats.look_ahead.y * (player.velocity.y \
					/ player.jump_stats.max_fall_speed), -size.y, size.y), camera_stats.damping.y * delta)
...
func _look_around(delta: float):
	if player.input_direction == 0.0:
		if Input.is_action_pressed("look_up"):
			looking_around = true
			position.y = lerpf(position.y, position.y - camera_stats.look_around_amount, camera_stats.damping.y * delta)
